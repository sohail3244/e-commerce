...
import { 
  ...
  Home, X, Map, ArrowLeft, PlusCircle 
} from "lucide-react";
import {
  addAddress,
  removeAddress,
...
export default function AddressPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
      ...
      {/* --- TOP NAVBAR (Synced with Order Details Page) --- */}
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        {/* Mobile-only Add Button (Visible when form is hidden) */}
        {!showForm && (
          <button 
            onClick={() => setShowForm(true)}
        ...
        {showForm && (
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] mb-12 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-xl font-black text-slate-800">New Address</h2>
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Fill in the delivery details</p>
              </div>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {addresses.length === 0 && !showForm && (
            <div className="col-span-full flex flex-col items-center justify-center py-24 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-900 font-black text-lg">No Addresses Saved Yet</p>
              <p className="text-slate-400 text-sm mb-6 text-center px-4">Add an address to speed up your checkout process.</p>
              <button
                onClick={() => setShowForm(true)}
                ...
                <div className="flex flex-col h-full relative z-10">
                  <div className="flex justify-between items-start mb-4">
                    <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      isSelected ? "bg-[#2A4150] text-white" : "bg-slate-100 text-slate-400"
                    }`}>
                      {isSelected ? "Default Destination" : "Saved Location"}
                    </div>
                    {isSelected && <CheckCircle2 className="text-[#2A4150]" size={22} />}
                  </div>

                  <div className="flex gap-4">
                      <div className={`mt-1 p-2 rounded-lg h-fit ${isSelected ? "bg-[#2A4150]/10 text-[#2A4150]" : "bg-slate-50 text-slate-400"}`}>
                          <MapPin size={18} />
                      </div>
                      <div>
                  ...
                  <div className="mt-8 flex items-center justify-between pt-4 border-t border-slate-50">
                    <span className={`text-[10px] font-black uppercase tracking-tighter ${isSelected ? "text-slate-800" : "text-slate-800"}`}>
                      {isSelected ? "Ready for Delivery" : "Select for Delivery"}
                    </span>
